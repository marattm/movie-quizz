...
myDB = pw.MySQLDatabase("marattm_monid_high_score",
...
class HighScore(pw.Model):
    class Meta:
        database = myDB
    # Definition de 3 colonnes
    player = pw.CharField()  # Stocke une chaine de caracteres
    game = pw.CharField()
    score = pw.BigIntegerField()  # Stocke en entier long
...
class Users(pw.Model):
    ...
    # Definition de 3 colonnes
    name = pw.CharField()           #nom image
    pwd = pw.CharField()            #adresse une image
    label_name = pw.CharField()     #type de l'image
    score = pw.BigIntegerField()    #ordre
    # le champ id est automatiquement cree


if __name__ == "__main__":
    # si ce fichier est execute a la main, alors creation de la table
    myDB.create_tables([HighScore],[Quizz])
